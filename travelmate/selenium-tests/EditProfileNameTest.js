...
(async function editProfileNameTest() {
  let driver;

  // User credentials
  const USER_EMAIL = "rrona2004@gmail.com";
  const USER_PASSWORD = "Rrona12?";
  ...
  try {
    console.log("🚀 Starting Edit Profile Name test...");
    ...
    const firstNameInputSelector = 'input[name="Name"]';
    console.log(
      `🔍 Looking for First Name input with selector: ${firstNameInputSelector}`
    );
    await fillInput(
      driver,
      By.css(firstNameInputSelector),
      `TestName${Date.now()}`
    );
    ...
    console.log(
      `🔍 Looking for Last Name input with selector: ${lastNameInputSelector}`
    );
    await fillInput(
      driver,
      By.css(lastNameInputSelector),
      `TestLastname${Date.now()}`
    );
    ...
    console.log("🎉 Edit Profile Name test completed successfully!");
  } catch (err) {
    console.error("❌ Edit Profile Name test failed:", err.message);
  } finally {
    if (driver) {
      try {
        await driver.quit();
